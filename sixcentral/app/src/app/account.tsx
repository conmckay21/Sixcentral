...
export default function Account() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      const [{ data: p }, { data: f }] = await Promise.all([
        supabase
          ...
          .select('handle, avatar_url, flair, bio, platform, psn_id, xbox_gamertag, ids_public, rank_id, is_staff, is_pro')
  ...
  async function save() {
    ...
    if (error) {
      ...
      flash(false, dobGate ? 'Public gamer IDs are an 18-plus feature. Add your date of birth on the website first.' : 'Could not save. Try again.');
  ...
  return (
    <SafeAreaView style={st.safe}>
      <ScrollView contentContainerStyle={st.pad} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <Pressable style={st.checkRow} onPress={() => setIdsPublic(!idsPublic)}>
        ...
        <Text style={st.foot}>Date of birth and password changes live on the website for now.</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
