  ...
  return (
    <SafeAreaView style={st.safe}>
      <ScrollView contentContainerStyle={st.pad} showsVerticalScrollIndicator={false}>
        ...
        <Text style={st.kicker}>{item.kicker || 'Guide'}</Text>
        <Text style={st.h1}>{item.title}</Text>
        <Text style={st.meta}>
          Updated {date} · {item.readingMins} min read
        </Text>

        {item.heroImage ? (
          <View style={st.heroWrap}>
            <Image source={{ uri: item.heroImage.src }} style={st.hero} resizeMode="cover" />
            {item.heroImage.credit ? <Text style={st.credit}>Image: {item.heroImage.credit}</Text> : null}
          </View>
        ) : (
        ...
        {item.youtubeId ? (
          <View style={st.videoWrap}>
            <WebView
              style={st.video}
              source={{ uri: `https://www.youtube-nocookie.com/embed/${item.youtubeId}` }}
              allowsFullscreenVideo
              mediaPlaybackRequiresUserAction
              javaScriptEnabled
              domStorageEnabled
            />
            <Text style={st.credit}>The SixCentral video guide</Text>
          </View>
        ) : null}
        ...
        <Text style={st.foot}>Checked in-game before it is published. sixcentral.co.uk/online</Text>
      </ScrollView>
    </SafeAreaView>
  );
}
...
function Reactions({ slug }: { slug: string }) {
  ...
  return (
    <View style={st.reactWrap}>
      <Text style={st.reactKicker}>Good guide?</Text>
...
const st = StyleSheet.create({
  ...
  videoWrap: { marginBottom: 18 },
  video: { width: '100%', aspectRatio: 16 / 9, borderRadius: 16, overflow: 'hidden', backgroundColor: C.surface },
